...
// recipes.forEach((val, index, arr) => {
//   console.log(val.name + " == " + val.cuisine);
// });

const resarr1 = recipes.map((val, ind, arr) => {
  const { name, cuisine } = val;
  return { name, cuisine };
  //   return { name: val.name, cuisine: val.cuisine };
});

const resarr = recipes.filter((val, ind, arr) => {
  const { name, rating } = val;
